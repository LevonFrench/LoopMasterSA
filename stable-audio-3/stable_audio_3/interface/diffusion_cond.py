...
def generate_cond(
    ...
    if save_to_dir and output_dir:
        try:
            ...
            shutil.copy2(output_filename, target_path)
            print(f"Saved copy of generated file to: {target_path}")
        except Exception as e:
            print(f"Error copying file to output directory {output_dir}: {e}")
...
def create_sampling_ui(stable_audio_3_model, default_prompt=None, default_output_dir="outputs"):
    global n_loras
    ...
    with gr.Row(equal_height=False):
        with gr.Column():
            ...
            with gr.Accordion("Output params", open=False):
                ...
                with gr.Row():
                    output_dir_input = gr.Textbox(label="Output directory", value=default_output_dir)
                    save_to_dir_checkbox = gr.Checkbox(label="Save to output directory", value=True)
...
def create_diffusion_cond_ui(model, gradio_title="", default_prompt=None, default_output_dir="outputs"):
    ...
    with gr.Blocks() as ui:
        ...
        with gr.Tab("Generation"):
            create_sampling_ui(model, default_prompt=default_prompt, default_output_dir=default_output_dir)
