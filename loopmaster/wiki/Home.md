...
> [!TIP]
> For a practical, feature-by-feature walk-through and step-by-step instructions on running and using the studio interface, check out the comprehensive [User Guide](file:///j:/projects/sa3/loopmaster/wiki/User-Guide.md).
...
A real-time audio visualizer panel sits between the transport bar and the track rows. Three canvases driven by the master output via Web Audio `AnalyserNode`:
...
LoopMaster SA3 relies on a combination of generative AI and customized Web Audio DSP modeling:
